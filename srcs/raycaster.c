...
/*   Updated: 2023/03/28 19:05:52 by lle-bret         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_verLine(t_data *data, t_raycaster *rc, int screenX)
{
	...
	rc->sideDist = set_vect(rc->step.x * (rc->map.x - rc->pos.x + (rc->step.x >= 0)) * rc->deltaDist.x,
		rc->step.y * (rc->map.y - rc->pos.y + (rc->step.y >= 0)) * rc->deltaDist.y);
	rc->hit = 0;
	while (rc->hit == 0)
		followRay(data, rc);
